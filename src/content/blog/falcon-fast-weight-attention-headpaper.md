...
这里εattn≥0是一个小稳定器。仅当读取规范化器时才使用规范化形式
...
和 ηt 得到的步长。步长为 ηt 的在线梯度下降的单步得出
德尔塔规则：
St =S t−1−ηt kt(S⊤
t−1kt−vt)⊤= (I−ηtktk⊤
t )St−1+ηtktv⊤
...
算法2并行化FALCON-2（Chunk-wise Forward，λt = 0）
...
表 1 在 FineWeb-Edu 上针对 50B 代币预算训练的具有 124M-130M 参数的模型比较。
越低越好。
模型困惑↓
维基百科。左键。精细教育。
124M-130M参数
...
表4 长上下文检索结果。124M-130M参数训练的模型比较
...
Albert Gu 和 Tri Dao。 Mamba：具有选择性状态空间的线性时间序列建模.arXiv
...
孙宇、李新浩、Karan Dalal、徐嘉瑞、Arjun Vikram、张庚瀚、Yann Dubois、Xinlei
...
有 dv 列，但它是一个高度优化的多 RHS 求解，其实质上更多
实践中对 GPU 友好。
...
转置等价形式。ParallelFlow 通常以共享右因子形式呈现
dωt =A tB⊤
t dt, dψt = ～AtB⊤
